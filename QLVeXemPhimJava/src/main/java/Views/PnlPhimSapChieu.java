...
public class PnlPhimSapChieu extends javax.swing.JPanel {

    private User currentUser = new User();
    public PnlPhimSapChieu(User user) {
        currentUser = user;
        initComponents();
        ...
        try {
            ...
            Logger.getLogger(PnlPhimSapChieu.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void showData(User user) throws SQLException, ClassNotFoundException{
        ...
        List<Movie> movies = moviesDAO.getMovieComingSoon();
        int num = 0;
        for(int i = 0; i < 4; i++){
            for(int j = 0; j< 5; j++ ){
                if (num >= movies.size()) {
