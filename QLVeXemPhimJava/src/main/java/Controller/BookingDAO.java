...
public class BookingDAO extends DatabaseConnection {
    ...
    public List<List<Integer>> getBookingMonthYear() throws SQLException, ClassNotFoundException {
        List<List<Integer>>  monthYears = new ArrayList<>();
        String query = "SELECT DISTINCT YEAR(booking_date) AS year, MONTH(booking_date) AS month FROM bookings WHERE is_deleted = 0";
        ...
        try (Connection conn = getConnection();
            ...
            while (rs.next()) {
                ...
                // Kết hợp tháng và năm thành chuỗi theo định dạng "YYYY-MM"
//                monthYears.add(String.format("%04d-%02d", year, month));
    ...
    public double getTotalRevenueByMonth(int year, int month) throws SQLException, ClassNotFoundException {
        double totalRevenue = 0.0;
        String query = "SELECT SUM(b.amount) AS total_ticket_sales " +
                       "FROM bookings b " +
        ...
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    totalRevenue = rs.getDouble("total_ticket_sales");
                }
            }
        }
