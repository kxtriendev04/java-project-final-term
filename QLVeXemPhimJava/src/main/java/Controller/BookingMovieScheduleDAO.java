...
public class BookingMovieScheduleDAO extends DatabaseConnection {
    ...
    public BookingMovieSchedule getBookingWithMoviesById(String bookingId) throws SQLException, ClassNotFoundException {
        BookingMovieSchedule bookingMovieSchedule = null;

        // SQL query để lấy dữ liệu từ các bảng bookings, movies và schedule theo booking_id
        String query = """
            SELECT 
                b.booking_id, 
                b.user_id, 
        ...
        try (Connection connection = getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                // Nếu có kết quả, gán dữ liệu vào đối tượng BookingMovieSchedule
                if (rs.next()) {
                    bookingMovieSchedule = new BookingMovieSchedule();
        ...
        // Trả về đối tượng BookingMovieSchedule (hoặc null nếu không tìm thấy)
        return bookingMovieSchedule;
    }
